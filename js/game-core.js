...
class GameObject {
    ...
    get obsolete() {
        ...
        for (let limitId in this.limit) {
            if (game.obj(limitId).count < this.limit[limitId]) { return false }
        }
...
class Game {
    ...
    loadData() {
        ...
        if (loadedData && loadedData !== null && loadedData.length % 4 == 0) {
            ...
            if (loadedData.worldId) {
                ...
                this.startNewRun(loadedData.worldId)                
                this.gameObjects.forEach(obj => { obj.onLoad(loadedData) })
            }
        }
        else { this.startNewRun('world1') }
    }
    ...
    buildData() {
        ...
        this.techs = this.gameObjects.filter(obj => { return obj.type == 'tech'})
        this.houses = this.gameObjects.filter(obj => { return obj.type == 'house'})
        this.resources = this.gameObjects.filter(obj => { return obj.type == 'resource'})
        this.townHalls = this.gameObjects.filter(obj => { return obj.type == 'townHall'})
    ...
    buildUI() {
        ...
        html = '<div class="row g-1">'
        this.townHalls.forEach(townHall => { html += getHtmlTownHall(townHall) })
        html += '</div>'
        ui.find('#townHalls').innerHTML = html        
        
        //--- houses

        html = '<div class="row g-1">'
        this.houses.forEach(house => { html += getHtmlBuilding(house) })
        html += '</div>'
        ...
        html = '<div class="row g-1">'
        this.techs.forEach(tech => { html += getHtmlTech(tech) })
        html += '</div>'
    ...
    refreshPage() {    
        ...
        ui.findAll('[data-show]').forEach(elem => {
            ...
            if (attr == 'townHall') {
                ...
                if (data > 0) { elem.style.display = 'block' }
                ...
                if (data > 0) { elem.style.display = 'block' }
                else if (game.obj(objId).unlocked == true && game.obj(objId).obsolete == false) { elem.style.display = 'block' }
                else { elem.style.display = 'none' }
            }
        ...
        ui.findAll('[data-display]').forEach(elem => {
            ...
            if (attr == 'count') {
                ...
                let data = Math.round(game.obj(objId).build.cost[value] + Number.EPSILON)

                if (data > game.obj(value).count) { elem.innerHTML = '<span class="text-danger">' + data + '</span>' }
                else { elem.innerHTML = '<span class="text-success">' + data + '</span>' }
            }
                ...
                let objId = ui.dataset(elem, 'object')
                let data = game.obj(objId).progress
    ...
    update(elapsedSeconds) {
        ...
        this.gameObjects.forEach(obj => {
            ...
            if (obj.producers.length > 0) {
                
                obj.count += obj.production * elapsedSeconds
            }
...
function getHtmlTownHall(townHall) {
    
    let html = ''
    
    html += '<div class="col-12" data-show="townHall" data-object="' + townHall.id + '">'
        html += '<div class="card card-body py-1 px-0 bg-transparent border-0">'
            html += '<div class="row gx-2">'
                html += '<div class="col-auto">'
                    html += '<img src="img/' + townHall.img + '" width="16px" />'
                html += '</div>'
                html += '<div class="col">'
                    html += '<span class="h6">' + trans.translate(townHall.id) + '</span>'
                html += '</div>'
                html += '<div class="col-auto">'
                    html += '<div class="row gx-2">'
                        for (let id in townHall.prod) {
                            html += '<div class="col-auto">'
                                html += '<div class="row gx-1">'
                                    ...
                                    html += '</div>'
                                    html += '<div class="col-auto">'
                                        html += '<span class="text-success">+' + townHall.prod[id] + ' <small class="opacity-50">/s</small></span>'
                                    html += '</div>'
                                html += '</div>'
                            html += '</div>'
                        }
...
function getHtmlBuilding(building) {
    ...
    html += '<div class="col-12" data-show="building" data-object="' + building.id + '">'
        html += '<div class="card">'
            html += '<button class="card-header collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse' + building.id + '" aria-expanded="false" aria-controls="collapse' + building.id + '">'
                html += '<div class="row gx-2 align-items-center">'
                    html += '<div class="col-auto">'
                        html += '<i class="fa-fw fas fa-caret-down"></i>'
                    html += '</div>'
                    html += '<div class="col">'
                ...
                html += '</div>'
            html += '</button>'
            html += '<div class="collapse" id="collapse' + building.id + '">'
                html += '<div class="card-body border-top">'
                    html += '<div class="row g-1">'
                                        ...
                                        for (let id in building.gain) {
                                            html += '<div class="col-auto">'
                                                html += '<div class="row gx-1">'
                                                    ...
                                                    html += '</div>'
                                                    html += '<div class="col-auto">'
                                                        html += '<span class="text-success">+' + building.gain[id] + '</span>'
                                                    html += '</div>'
                                                html += '</div>'
                                            html += '</div>'
                                        }
                                        ...
                                        for (let id in building.prod) {
                                            html += '<div class="col-auto">'
                                                html += '<div class="row gx-1">'
                                                    ...
                                                    html += '</div>'
                                                    html += '<div class="col-auto">'
                                                        html += '<span class="text-success">+' + building.prod[id] + ' <small class="opacity-50">/s</small></span>'
                                                    html += '</div>'
                                                html += '</div>'
                                            html += '</div>'
                                        }
                        ...
                        if (building.build) {
                            if (building.build.cost) {
                                html += '<div class="col-12">'
                                    html += '<div class="row gx-2">'
                                        ...
                                        html += '<div class="col-auto">'
                                            html += '<div class="row gx-2">'
                                            for (let id in building.build.cost) {
                                                html += '<div class="col-auto">'
                                                    html += '<div class="row gx-1">'
                                                        html += '<div class="col-auto">'
                                                            html += '<img src="img/' + game.obj(id).img + '" width="16px" />'
                                                        html += '</div>'
                                                        html += '<div class="col-auto">'
                                                            html += '<span data-display="cost" data-value="' + id + '"></span>'
                                                        html += '</div>'
                                                    html += '</div>'
                                                html += '</div>'
                                            }
                            ...
                            html += '<div class="col-12 mt-3" data-show="statusIdle">'
                                html += '<div class="row gx-2 align-items-center">'
                                    html += '<div class="col">'
                                        html += '<button type="button" class="btn btn-danger" data-check="canDelete" onclick="clickDelete(\'' + building.id + '\')">'
                                            html += 'Delete'
                                        html += '</button>'
                                    html += '</div>'
                                    if (building.build.time) {
                                        html += '<div class="col-auto">'
                                            html += '<span>' + formatTime(building.build.time) + '</span>'
                                        html += '</div>'
                                    }
                                    html += '<div class="col-auto">'
                                        html += '<button type="button" class="btn btn-primary" data-check="canBuild" onclick="clickBuild(\'' + building.id + '\')">'
                                            html += 'Build'
                                        html += '</button>'
                                    html += '</div>'
                                html += '</div>'
                            html += '</div>'
                            html += '<div class="col-12 mt-3" data-show="statusBuilding">'
                                html += '<div class="row gx-2 align-items-center">'
                                    html += '<div class="col">'
                                    html += '</div>'
...
function getHtmlTech(tech) {
    ...
    html += '<div class="col-12" data-show="tech" data-object="' + tech.id + '">'
        html += '<div class="card">'
            html += '<button class="card-header collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse' + tech.id + '" aria-expanded="false" aria-controls="collapse' + tech.id + '">'
                html += '<div class="row gx-2 align-items-center">'
                    html += '<div class="col-auto">'
                        html += '<i class="fa-fw fas fa-caret-down"></i>'
                    html += '</div>'
                ...
                html += '</div>'
            html += '</button>'
            html += '<div class="collapse" id="collapse' + tech.id + '">'
                html += '<div class="card-body border-top">'
                    html += '<div class="row g-1">'
                        if (tech.build) {
                            if (tech.build.cost) {
                            ...
                            html += '<div class="col-12 mt-3" data-show="statusIdle">'
                                html += '<div class="row gx-2 align-items-center">'
                                    ...
                                    html += '</div>'
                                    html += '<div class="col-auto">'
                                        html += '<button type="button" class="btn btn-primary" data-check="canBuild" onclick="clickBuild(\'' + tech.id + '\')">'
                                            html += 'Build'
                                        html += '</button>'
                                    html += '</div>'
                                html += '</div>'
